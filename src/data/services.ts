...
const serviceLinesPt: ServiceLine[] = [
  ...
  },
  {
    ...
    title: 'Arquitetura, DevOps, Cloud e FinOps',
    subtitle: 'Performance, fluidez, resiliência e escalabilidade',
    problem: 'Estruturamos arquiteturas cloud-native, estratégias multicloud, esteiras DevOps, FinOps e integrações via API (incluindo Open Finance) para suportar crescimento, alta disponibilidade, otimização de custos, ciclos mais rápidos e conectividade segura entre ecossistemas.',
    target: 'Times e empresas que precisam ganhar escala, resiliência e previsibilidade com arquitetura moderna, cloud, automação de delivery, eficiência financeira e integrações robustas.',
    icon: 'landmark',
    color: 'from-fuchsia-500 to-fuchsia-700',
    tags: ['Arquitetura', 'Cloud-Native', 'Multicloud', 'DevOps', 'CI/CD', 'Kubernetes', 'FinOps', 'Observabilidade', 'APIs', 'Open Finance', 'Integração'],
    offerDetail: {
      valueProposition: 'Construir plataformas cloud-native resilientes, escaláveis e eficientes — alinhando arquitetura moderna, estratégias multicloud, práticas DevOps, FinOps e integrações via API ao ritmo real do negócio. O mercado global de cloud computing alcançou US$ 832 bilhões em 2025 e 92% dos workloads estarão em cloud até 2028.',
      metrics: [
        { value: '$832B', label: 'Mercado Cloud 2025' },
        { value: '-35%', label: 'Redução de Custo Cloud (FinOps)' },
      ...
      differentials: [
        ...
        'FinOps como disciplina, não apenas ferramenta — redução média de 35% em custo cloud.',
        ...
        'APIs, eventos e integrações complexas com experiência prática em Open Finance e ambientes regulados.',
        'Governança de APIs desde o design até a operação — integração pensada como produto.',
      ],
    },
  },
  {
...
const serviceLinesEn: ServiceLine[] = [
  {
    ...
    subtitle: 'Cost reduction and Technology Evolution',
  ...
  {
    id: 'arquitetura-devops',
    title: 'Architecture, DevOps, Cloud & FinOps',
    subtitle: 'Performance, fluidity, resilience, and scalability',
    problem: 'We design cloud-native architectures, multicloud strategies, DevOps pipelines, FinOps, and API integrations (including Open Finance) to support growth, high availability, cost optimization, faster cycles, and secure connectivity across ecosystems.',
    target: 'Teams and companies that need to gain scale, resilience, and predictability with modern architecture, cloud, delivery automation, financial efficiency, and robust integrations.',
    icon: 'landmark',
    color: 'from-fuchsia-500 to-fuchsia-700',
    tags: ['Architecture', 'Cloud-Native', 'Multicloud', 'DevOps', 'CI/CD', 'Kubernetes', 'FinOps', 'Observability', 'APIs', 'Open Finance', 'Integration'],
    offerDetail: {
      valueProposition: 'Build resilient, scalable, and efficient cloud-native platforms — aligning modern architecture, multicloud strategies, DevOps practices, FinOps, and API integrations to the real pace of business. The global cloud computing market reached $832 billion in 2025, and 92% of workloads will be on cloud by 2028.',
      metrics: [
        { value: '$832B', label: 'Cloud Market 2025' },
        { value: '-35%', label: 'Cloud Cost Reduction (FinOps)' },
      ...
      differentials: [
        ...
        'FinOps as a discipline, not just a tool — average 35% cloud cost reduction.',
        ...
        'APIs, events, and complex integrations with hands-on experience in Open Finance and regulated environments.',
        'API governance from design to operations — integration designed as a product.',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    subtitle: 'Intelligence for decision making',
...
/**
 * Linhas de serviço que NÃO aparecem na grade orbital (8 cards), mas continuam
 * ativas como ofertas flagship em SectionOffersFlag/SectionHome e expõem
 * Deep Dive próprio quando o usuário clica em "Ver detalhes completos".
 *
 * Mantemos os objetos completos aqui porque o DeepDiveModal precisa de
 * title/subtitle/problem/target/offerDetail para renderizar o cabeçalho.
 */
const flagshipServiceLinesPt: ServiceLine[] = [
  {
...
const flagshipServiceLinesEn: ServiceLine[] = [
  {
    id: 'ai-augmented-squad',
    title: 'AI Squad',
...
export function getFlagshipServiceLines(lang: Language): ServiceLine[] {
  return lang === 'en' ? flagshipServiceLinesEn : flagshipServiceLinesPt
}

/**
 * Busca uma ServiceLine pelo id em qualquer um dos arrays disponíveis
 * (grade principal de 8 cards + flagship-only como AI Squad e FourBlox).
 * Útil para handlers que recebem o id via deepDiveHint vindo de outra seção.
 */
export function findServiceLineById(lang: Language, id: string): ServiceLine | undefined {
  return (
    getServiceLines(lang).find(s => s.id === id) ??
    getFlagshipServiceLines(lang).find(s => s.id === id)
  )
}
